...
def prepare_tree_classifier():

    global ENABLE_CLASSIFICATION
    print("Checking for tree classifier...")

    try:
        # Wczytywanie klasyfikatora
        tree = load('./static/tree.joblib')
        tree_features = load('./static/tree_features.joblib')
        stdsc = load('./static/tree_stdsc.joblib')
    except FileNotFoundError as ex:
        print(f"Cannot load classifier, features or scaler, creating new one...")
    else:
        print("Tree loaded successfully!")
        print(f"Tree info: {tree.get_params()}")
        print(f"Usef features: {tree_features}")
        ENABLE_CLASSIFICATION = True
        return
    ...
    try:
        ...
        print("Dataset for tree not found!")
        ENABLE_CLASSIFICATION = False
        return
        ...
        for md in [*np.arange(3, 10), None]:
            ...
            for num_of_features in range(1, rank_range):
                ...
                for train_index, test_index in splitted_data:
                    ...
                    # inicjalizacja klasyfikatora
                    tree = DecisionTreeClassifier(criterion=crit, max_depth=md, random_state=0)

                    # uczenie klasyfikatora za pomocą danych trenujących
                    tree.fit(X_train, y_train)

                    # zliczanie średniej dokładnośći
                    iteration_score += (tree.score(X_test, y_test) / 10)

                if tree.score(X_test, y_test) > best_score:
                    best_svm = tree
                    best_score = tree.score(X_test, y_test)
                    best_features = zipped_list_columns[:num_of_features]
    ...
    dump(best_svm, r"./static/tree.joblib")
    dump(best_features, r"./static/tree_features.joblib")
    dump(best_stdsc, r"./static/tree_stdsc.joblib")
...
def classify_image(img):
    ...
    try:
        ...
        try:
            features = load('./static/tree_features.joblib')
            classifier = load('./static/tree.joblib')
            stdsc = load('./static/tree_stdsc.joblib')
        except FileNotFoundError as ex:
